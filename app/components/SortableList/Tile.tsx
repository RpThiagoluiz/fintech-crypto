...
export const Tile = ({ id }: TileProps) => {
   const balanceStore = useBalanceStore();

   const totalNegativeAmounts = balanceStore.transactions.reduce((acc, cur) => {
      if (cur.amount < 0) {
            ...
            <View>
               ...
               {balanceStore.transactions.length > 0 && (
                  <>
                     ...
                     >
                        {balanceStore.transactions[balanceStore.transactions.length - 1].amount}$
                     </Text>
                     <Text style={{ color: Colors.gray, fontWeight: 'bold', fontSize: 16 }}>
                        {balanceStore.transactions[balanceStore.transactions.length - 1].title}
                     </Text>
                  </>
               )}
            </View>
